...
/backend              - Go API сервер
  ...
  ├── api/chat.go          - обработчик /api/chat (SSE streaming)
  ├── gigachat/client.go   - клиент GigaChat API
  ├── config/config.go     - загрузка конфигурации
  └── static/              - встроенный frontend (после сборки)

/frontend             - Svelte приложение
  └── src/
      ├── App.svelte            - главный компонент
      ├── lib/api.ts            - API клиент (async generator для SSE)
      ├── lib/ChatMessage.svelte
      └── lib/ChatInput.svelte
...
### Backend
- HTTP сервер с endpoint `/api/chat` (POST)
- SSE streaming ответов от GigaChat
- Получение OAuth токена из Authorization Key
- CORS обработка
- Раздача статического контента

### Frontend
- Чат-интерфейс в стиле Google AI
- Streaming отображение ответов через async generator
- TypeScript с строгими типами

### CI/CD
- Автоматическая сборка frontend → копирование в backend/static → сборка Go → деплой на VPS

## Возможные улучшения

1. Хранение истории сообщений (in-memory или БД)
2. Аутентификация пользователей
3. Мониторинг и структурированное логирование
4. Unit/компонентные тесты
